...
const createPhotographersCard = photographers => {
...
const insertPhotographersCard = photographersCard => {
  const placeHolder = uiMain.querySelector(".section-photographers")
  placeHolder.insertAdjacentHTML("beforeend", photographersCard)
}
...
const initPhotographersCard = (apiUrl, photographers) => {
  document.querySelectorAll(".card-photographer[data-tag-filtrable]").forEach(element => element.remove())

  const photographersCard = createPhotographersCard(photographers)
  insertPhotographersCard(photographersCard)
...
export default initPhotographersCard
